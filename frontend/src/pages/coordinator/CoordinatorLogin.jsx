...
const CoordinatorLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post("/game-coordinator/login", formData);

      if (response.data.success) {
        // Store token and coordinator data
        localStorage.setItem("coordinatorToken", response.data.token);
        localStorage.setItem(
          "coordinatorData",
          JSON.stringify(response.data.coordinator),
        );
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-[#0a0a18] to-black flex items-center justify-center p-4 overflow-hidden relative">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm">Game Coordinator Portal</p>
        </div>
        ...
        >
          ...
          <p className="text-gray-400 mb-6">
            Sign in to view sports registrations
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full py-3 bg-gradient-to-r from-green-500 to-blue-500 text-white font-bold rounded-lg shadow-lg hover:shadow-green-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-rajdhani text-lg"
            >
          ...
          <div className="mt-6 pt-6 border-t border-gray-700/50">
            <Link
              to="/home"
              className="block text-center text-gray-400 hover:text-green-400 transition-colors"
            >
              ← Back to Home
            </Link>
          </div>
        ...
        <p className="text-center text-gray-500 text-sm mt-8">
          © 2026 Zenith. Secure Coordinator Access.
        </p>
      </motion.div>
    </div>
  );
